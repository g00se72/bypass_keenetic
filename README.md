...
> [!WARNING]
> Временно рекомендую для включения/выключения DNS Ovveride не использовать бота. Обнаружил странное поведение (по крайней мере у себя) - после включения или отключения не устанавливается провайдерское PPPoE соединение. Помогает любое изменение, например, установить неправильный пароль, сохранить и затем вернуть правильный. После этого сразу все восстанавливается. Пока не удалось установить в чем проблема и когда она стала проявляться (возможно, как то связано с новыми версиями прошивки или еще чем-то)
>
> в CLI роутера (или ssh подключение на 22 порт)\
> ВКЛ
> ```bash
> opkg dns-override
