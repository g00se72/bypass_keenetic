...
def create_keys_bridges_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("Tor", "Vless", "Trojan", "Shadowsocks")
    markup.row("🔙 Назад")
    return markup
...
def handle_keys_bridges_selection(message):
    if message.text == 'Tor':
        set_level_and_reply(message.chat.id, 8, "🔑 Скопируйте мосты сюда", create_back_menu())
...
def handle_tor_manually(message):
    ...
    time.sleep(1)
    bot.send_message(message.chat.id, '✅ Мосты Tor успешно обновлены')
...
def handle_shadowsocks(message):
    ...
    time.sleep(1)
    bot.send_message(message.chat.id, '✅ Ключ Shadowsocks обновлен')
...
def handle_vless(message):
    vless(message.text)
    time.sleep(1)
    bot.send_message(message.chat.id, '✅ Ключ Vless обновлен')
...
def handle_trojan(message):
    trojan(message.text)    
    time.sleep(1)
    bot.send_message(message.chat.id, '✅ Ключ Trojan обновлен')
